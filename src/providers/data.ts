	...
	login(username, password) {
		...
		return new Promise(resolve => {
			if (username == "trivento@trivento.nl" && password == "Trivento"){
				...
				resolve({ success: false, errorMessage: "Gebruikersnaam of wachtwoord is niet correct." });
			}
			// this.http.post("https://lutsoft.nl/trivento/api/login", JSON.stringify(json)).subscribe(success => {
			...
			// 		}
			// 	} else {
			// 		resolve({ success: false, errorMessage: "Inloggen mislukt." });
			// 	}
			// }, error => {
		...
		return new Promise(resolve => {
			this.storage.get("employees").then(data => {						
				if (data) {					
					...
					this.employees = this.clone(this.defaultEmployees);
					resolve(this.employees);
					// this.http.get("https://lutsoft.nl/trivento/api/data").subscribe(success => {
					// 	if (success) {
